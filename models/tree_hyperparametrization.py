...
def hyperparameter_tuning_cv():
    ...
    for fold, (train_idx, test_idx) in enumerate(splits, start=1):
        ...
        elapsed_time_gridsearch = elapsed_time_min_sec(
            time.perf_counter(), start_grid_search)
    ...
    elapsed_time_bestparam_search = elapsed_time_min_sec(
        time.perf_counter(), start_bestparam_search)
    config_summary = {}
...
def evaluate_fixed_params(params):
    ...
    for fold, (train_idx, test_idx) in enumerate(splits, start=1):
        ...
        elapsed_time_fit = elapsed_time_min_sec(
            time.perf_counter(), start_training_fit)
    ...
    elapsed_evaluation_time = elapsed_time_min_sec(
        time.perf_counter(), start_evaluation_time)
    accs = [r['accuracy'] for r in results]
    f1s = [r['f1_score'] for r in results]

...
def hyperparameter_tuning_cv():
    ...
    for fold, (train_idx, test_idx) in enumerate(splits, start=1):
        ...
        print(
            f"Fold {fold:02d}: Acc={acc:.4f}, F1={f1:.4f}, Best params: {grid_search.best_params_}", flush=True)
        print(
            f"Tempo: {elapsed_time_gridsearch[0]}min {elapsed_time_gridsearch[1]:.2f}s", flush=True)
    ...
    print(
        f"Tempo Total de Busca: {elapsed_time_bestparam_search[0]}min {elapsed_time_bestparam_search[1]:.2f}s", flush=True)
...
def evaluate_fixed_params(params):
    ...
    for fold, (train_idx, test_idx) in enumerate(splits, start=1):
        ...
        print(
            f"Fold {fold:02d}: Acc={acc:.4f}, F1={f1:.4f}, Time: {elapsed_time_fit[0]}min {elapsed_time_fit[1]:.2f}s", flush=True)
    ...
    print(
        f"Tempo de Avaliação: {elapsed_evaluation_time[0]}min {elapsed_evaluation_time[1]:.2f}s", flush=True)
